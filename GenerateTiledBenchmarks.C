...
void generateTiledProg(
  ...
  SageInterface::loopTiling(loopNest, loopDepth, 5);

  backend(project);

  // Hacky solution to generate multiple tiled programs for test case
  const string mvBinary = "mv a.out " + to_string(counter) + ".out";
  const string mvSrc = "mv rose_symm.c rose_symm_" + to_string(counter) + ".c";
...
//        generateTiledProg(argc, argv, fl, 1, counter);
//        counter++;

        SageInterface::loopTiling(fl, 1, 10);
